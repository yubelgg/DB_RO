...
#include "occ/coordinator_enhanced.h"
#include "occ/scheduler_enhanced.h"
#include "occ/tx.h"
#include "occ/tx_enhanced.h"
...
Frame *Frame::RegFrame(int mode, function<Frame *()> frame_init) {
...
Frame *Frame::GetFrame(string name) { return GetFrame(Name2Mode(name)); }

Frame *Frame::RegFrame(int mode, vector<string> names,
...
mdb::Row *Frame::CreateRow(const mdb::Schema *schema, vector<Value> &row_data) {
...
Coordinator *Frame::CreateCoordinator(cooid_t coo_id, Config *config,
                                      int benchmark,
  ...
  switch (mode) {
  case MODE_2PL:
    coo = new Coordinator2pl(coo_id, benchmark, ccsi, id);
    ((Coordinator *)coo)->txn_reg_ = txn_reg;
    break;
  case MODE_OCC:
  case MODE_RPC_NULL:
    coo = new CoordinatorOcc(coo_id, benchmark, ccsi, id);
    ((Coordinator *)coo)->txn_reg_ = txn_reg;
    break;
  case MODE_OCC_ENHANCED:
    coo = new CoordinatorOccEnhanced(coo_id, benchmark, ccsi, id);
    ((Coordinator *)coo)->txn_reg_ = txn_reg;
    break;
  // Slim build: RCC and Snow protocols excluded
  // case MODE_RCC:
  ...
  //   break;
  case MODE_MDCC:
    //      coo = (Coordinator*)new mdcc::MdccCoordinator(coo_id, id, config,
    //      ccsi);
    break;
  case MODE_NONE:
  default:
    coo = new CoordinatorNone(coo_id, benchmark, ccsi, id);
    ((Coordinator *)coo)->txn_reg_ = txn_reg;
    break;
  }
  coo->frame_ = this;
...
Communicator *
Frame::CreateCommo(rusty::Arc<PollThreadWorker> poll_thread_worker) {
  commo_ = new Communicator(poll_thread_worker);
  return commo_;
}

shared_ptr<Tx> Frame::CreateTx(epoch_t epoch, txnid_t tid, bool ro,
                               TxLogServer *mgr) {
  shared_ptr<Tx> sp_tx;
  ...
  case MODE_OCC:
    ...
    break;
  case MODE_OCC_ENHANCED:
    sp_tx.reset(new TxOccEnhanced(epoch, tid, mgr));
    break;
  // Slim build: RCC and Snow protocols excluded
  // case MODE_RCC:
...
vector<rrr::Service *>
Frame::CreateRpcServices(uint32_t site_id, TxLogServer *dtxn_sched,
  ...
  default:
    result.push_back(
        new ClassicServiceImpl(dtxn_sched, poll_thread_worker, scsi));
    break;
  }
  return result;
}
map<string, int> &Frame::FrameNameToMode() {
  static map<string, int> frame_name_mode_s = {
      ...
      {"2pl", MODE_2PL},
      {"occ", MODE_OCC},
      {"occ_enhanced", MODE_OCC_ENHANCED},
      {"snow", MODE_RO6},
